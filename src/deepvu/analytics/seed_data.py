"""Seed in-memory DuckDB with realistic ad analytics dummy data for dev."""
...
def seed_analytics(conn: duckdb.DuckDBPyConnection) -> None:
    """Populate the in-memory DuckDB connection with dummy ad analytics data."""
    ...
    conn.execute("""
        CREATE TABLE campaign_metrics (
            advertiser_id VARCHAR, campaign VARCHAR, sub_campaign VARCHAR,
            plan_line_item VARCHAR, channel VARCHAR, currency VARCHAR,
            spends DOUBLE, impressions BIGINT, clicks BIGINT, ctr DOUBLE,
            views BIGINT, vtr DOUBLE, conversions BIGINT, cpm DOUBLE, cpc DOUBLE
        )
    """)
    ...
    for c in campaigns:
        conn.execute("INSERT INTO campaign_metrics VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", [ADV, *c])
    # Second advertiser (RLS should block these)
    for c in campaigns[:3]:
        conn.execute("INSERT INTO campaign_metrics VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", ["ADV_OTHER", *c])

    # ── YouTube metrics ──────────────────────────
    conn.execute("""
        CREATE TABLE youtube_metrics (
            advertiser_id VARCHAR, campaign VARCHAR, sub_campaign VARCHAR,
            plan_line_item VARCHAR, currency VARCHAR, spends DOUBLE,
            impressions BIGINT, views BIGINT, vtr DOUBLE, clicks BIGINT,
            ctr DOUBLE, earned_views BIGINT, earned_subscribers BIGINT
        )
    """)
    ...
    for yt in [
        ...
        conn.execute("INSERT INTO youtube_metrics VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)", [ADV, *yt])

    # ── Reach & Frequency ────────────────────────
    conn.execute("""
        CREATE TABLE reach_frequency (
            advertiser_id VARCHAR, sub_campaign VARCHAR, plan_line_item VARCHAR,
            frequency INTEGER, reach BIGINT, impressions BIGINT,
            currency VARCHAR, spends DOUBLE
        )
    """)
    ...
    for rf in rf_data:
        conn.execute("INSERT INTO reach_frequency VALUES (?,?,?,?,?,?,?,?)", [ADV, *rf])

    # ── Device Type ──────────────────────────────
    conn.execute("""
        CREATE TABLE device_metrics (
            advertiser_id VARCHAR, sub_campaign VARCHAR, plan_line_item VARCHAR,
            device_type VARCHAR, currency VARCHAR, spends DOUBLE,
            impressions BIGINT, clicks BIGINT, ctr DOUBLE,
            views BIGINT, vtr DOUBLE
        )
    """)
    ...
    weights = {"Mobile": 0.45, "Desktop": 0.30, "Tablet": 0.10, "Connected TV": 0.10, "Smart TV": 0.05}

    for sc, pli in sub_campaigns:
        for dev in devices:
            w = weights[dev]
            spends = round(random.uniform(800, 4000) * w, 2)
            imps = int(random.uniform(50000, 200000) * w)
            clicks = int(imps * random.uniform(0.003, 0.02))
            ctr = round(clicks / imps * 100, 2) if imps else 0
            views = int(imps * random.uniform(0.1, 0.4)) if "Video" in pli else int(imps * 0.05)
            vtr = round(views / imps * 100, 2) if imps else 0
            conn.execute("INSERT INTO device_metrics VALUES (?,?,?,?,?,?,?,?,?,?,?)",
                         [ADV, sc, pli, dev, "USD", spends, imps, clicks, ctr, views, vtr])

    # ── Geo Trends — Region ──────────────────────
    conn.execute("""
        CREATE TABLE geo_region_metrics (
            advertiser_id VARCHAR, sub_campaign VARCHAR, plan_line_item VARCHAR,
            region VARCHAR, currency VARCHAR, spends DOUBLE,
            impressions BIGINT, cpm DOUBLE
        )
    """)

    regions = [
        ("California", 18500.00, 1250000), ("New York", 15200.00, 980000),
        ("Texas", 12800.00, 920000), ("Florida", 10500.00, 780000),
        ("Illinois", 8200.00, 620000), ("Pennsylvania", 6800.00, 520000),
        ("Ohio", 5400.00, 430000), ("Georgia", 4800.00, 380000),
        ("North Carolina", 4200.00, 340000), ("Michigan", 3600.00, 290000),
        ("New Jersey", 3200.00, 260000), ("Virginia", 2800.00, 230000),
        ("Washington", 2500.00, 210000), ("Arizona", 2200.00, 180000),
        ("Massachusetts", 2000.00, 170000),
    ]
    for reg, spends, imps in regions:
        cpm = round(spends / imps * 1000, 2)
        conn.execute("INSERT INTO geo_region_metrics VALUES (?,?,?,?,?,?,?,?)",
                     [ADV, "Brand Awareness", "Display - Premium", reg, "USD", spends, imps, cpm])
        conn.execute("INSERT INTO geo_region_metrics VALUES (?,?,?,?,?,?,?,?)",
                     [ADV, "Retargeting", "Display - Retarget", reg, "USD", round(spends * 0.6, 2), int(imps * 0.5), round(spends * 0.6 / (imps * 0.5) * 1000, 2)])

    # ── Geo Trends — City ────────────────────────
    conn.execute("""
        CREATE TABLE geo_city_metrics (
            advertiser_id VARCHAR, sub_campaign VARCHAR, plan_line_item VARCHAR,
            city VARCHAR, currency VARCHAR, spends DOUBLE,
            impressions BIGINT, cpm DOUBLE
        )
    """)

    cities = [
        ("Los Angeles", 8200.00, 520000), ("New York City", 7800.00, 480000),
        ("Chicago", 5400.00, 380000), ("Houston", 4600.00, 340000),
        ("Phoenix", 3800.00, 280000), ("Philadelphia", 3200.00, 250000),
        ("San Antonio", 2800.00, 220000), ("San Diego", 2500.00, 200000),
        ("Dallas", 2200.00, 180000), ("San Jose", 2000.00, 160000),
        ("Austin", 1800.00, 150000), ("Jacksonville", 1500.00, 130000),
        ("San Francisco", 1400.00, 110000), ("Columbus", 1200.00, 100000),
        ("Indianapolis", 1100.00, 95000), ("Charlotte", 1000.00, 85000),
        ("Seattle", 950.00, 80000), ("Denver", 900.00, 75000),
        ("Boston", 850.00, 70000), ("Nashville", 800.00, 65000),
    ]
    for city, spends, imps in cities:
        cpm = round(spends / imps * 1000, 2)
        conn.execute("INSERT INTO geo_city_metrics VALUES (?,?,?,?,?,?,?,?)",
                     [ADV, "Brand Awareness", "Display - Premium", city, "USD", spends, imps, cpm])
        conn.execute("INSERT INTO geo_city_metrics VALUES (?,?,?,?,?,?,?,?)",
                     [ADV, "Retargeting", "Display - Retarget", city, "USD", round(spends * 0.5, 2), int(imps * 0.4), round(spends * 0.5 / (imps * 0.4) * 1000, 2)])

    # ── Placements ───────────────────────────────
    conn.execute("""
        CREATE TABLE placement_metrics (
            advertiser_id VARCHAR, placement VARCHAR, currency VARCHAR,
            spends DOUBLE, impressions BIGINT, clicks BIGINT, ctr DOUBLE,
            views BIGINT, vtr DOUBLE, cpm DOUBLE
        )
    """)
    ...
    for pl in [
        ...
        conn.execute("INSERT INTO placement_metrics VALUES (?,?,?,?,?,?,?,?,?,?)", [ADV, *pl])

    # ── Creative Performance ─────────────────────
    conn.execute("""
        CREATE TABLE creative_metrics (
            advertiser_id VARCHAR, sub_campaign VARCHAR, plan_line_item VARCHAR,
            creative VARCHAR, creative_name VARCHAR, format VARCHAR,
            concept VARCHAR, size VARCHAR, currency VARCHAR,
            spends DOUBLE, impressions BIGINT, cpm DOUBLE,
            clicks BIGINT, ctr DOUBLE
        )
    """)
    ...
    for cr in [
        ...
        conn.execute("INSERT INTO creative_metrics VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)", [ADV, *cr])
